...
class Mapper:
    ...
    def __init__(self, endpoint, api_key, in_file='', out_file='',
        ...
        if MAX_INPUTS_PER_QUERY < num_requests_parallel:
            logger.info('The of inputs to process per API request was set to its max of {}.'.format(MAX_INPUTS_PER_QUERY))
    ...
    def _load_inputs(self):
        ...
        logger.info("Number of input strings in ingested file: {}".format(len(raw_inputs)))
    ...
    def query_api(self, inputs):
        ...
        logger.debug('Timeout for these inputs: {}'.format(timeout))
    ...
    def bulk(self):
        ...
        if not self.force_reprocess:
            ...
            if len(processed_inputs):
                logger.info('Loaded {} results a previous run.'.format(len(processed_inputs)))
        ...
        for i, chunk in enumerate(_chunks(raw_inputs, self.inputs_per_request)):
            ...
            logger.info("Number of threads in the chunk: {}".format(num_threads))
            ...
            for _ in range(max_timeout_retry + 1):
                ...
                if any(self.has_key_limit(response[0]['Company Name']) for response in list_json_response):
                    ...
                    if num_threads > 4:
                        ...
                        num_threads -= 1
                    logger.info("Number of threads in the chunk after number of requests limitation error for key: {}".format(num_threads))
                    logger.info("Continue in {} seconds".format(self.N_sec_sleep_key_limit))
                ...
                if any(self.has_timeout(response[0]['Company Name']) for response in list_json_response):
                    ...
                    if num_threads > 4:
                        ...
                        num_threads -= 1
                    logger.info("Number of threads in the chunk after timeout error: {}".format(num_threads))
    ...
    def write_csv(self, one_json_response):
        ...
        for result in one_json_response:
            ...
            # non_changing_keys_dic={key: value for key, value in result.items() if key in non_changing_keys}
            # changing_dic={
            #     'Date & Time': date_time,
            ...
            # csv_row = non_changing_keys_dic.update(changing_dic)
            ...
            logger.debug('Wrote results for {}'.format(result["Original Input"]))

        logger.info('Wrote results to {}'.format(self.out_file_location))
